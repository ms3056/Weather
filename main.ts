...
function isHighHumidity(humidity: number): boolean {
    return humidity >= 80; // Adjust the humidity threshold as needed
}
...
function isHotTemperature(feelslike_c: number, feelslike_f: number): boolean {
    ...
    const hotTemperatureThresholdC = 29; // Celsius
    const hotTemperatureThresholdF = 86; // Fahrenheit

    return feelslike_c >= hotTemperatureThresholdC || feelslike_f >= hotTemperatureThresholdF;
}
...
export default class ObsidianWeatherPlugin extends Plugin {
    ...
    createWeatherHTML(data: WeatherAPIResponse): string {
        ...
        umbrellaIcon.style.display = daily_will_it_rain === 1 ? 'block' : 'none'; // Display the umbrella icon if daily_will_it_rain is 1, otherwise hide it
        ...
        setIcon(humidityIcon, isHighHumidity(Number(current.humidity)) && !isHotTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) ? 'droplets' : '');
        humidityIcon.style.display = isHighHumidity(Number(current.humidity)) && !isHotTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) ? 'block' : 'none';
