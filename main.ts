...
function describeAirQuality(data: AirQuality): {
	emoji: string;
	text: string;
		...
		const unsafePollutants = [
			...
			(pollutant) =>
				calculatePollutantAQI(
					pollutant.value,
					pollutant.name.toLowerCase()
				) >= 100
		);

		unsafeDescriptions = unsafePollutants
			.map(
				(pollutant) =>
					`${pollutant.name}: ${pollutant.value.toFixed(0)} µg/m³`
			)
			.join(", ");
	}
	...
	return {
		emoji: airQualityEmoji,
		text: airQualityText,
		mainContributors: unsafeDescriptions,
	};
}
...
function isColdTemperature(feelslike_c: number, feelslike_f: number): boolean {
	return feelslike_c <= 10 || feelslike_f <= 50; // Adjust the temperature thresholds as needed
}

function isHighHumidity(humidity: number): boolean {
	return humidity >= 60; // Adjust the humidity threshold as needed
}

function isHotTemperature(feelslike_c: number, feelslike_f: number): boolean {
	// Adjust the temperature threshold as per your definition of a hot temperature
	const hotTemperatureThresholdC = 19; // Celsius
	const hotTemperatureThresholdF = 86; // Fahrenheit

	return (
		feelslike_c >= hotTemperatureThresholdC ||
		feelslike_f >= hotTemperatureThresholdF
	);
}
...
export default class ObsidianWeatherPlugin extends Plugin {
	...
	async refreshWeather() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch weather data. Status: ${response.status} ${response.statusText}`
				);
			}
	...
	scheduleRefresh() {
		...
		if (this.settings.refreshRate !== 999) {
			this.refreshTimer = setTimeout(
				() => this.refreshWeather(),
				this.settings.refreshRate * 60 * 1000
			);
		}
	}
	...
	createWeatherHTML(data: WeatherAPIResponse): string {
		...
		const {
			emoji: airQualityEmoji,
			text: airQualityText,
			mainContributors: unsafeContributors,
		} = describeAirQuality(air_quality);
		...
		if (forecastDay1) {
			date_day1 = new Date(
				forecastDay1.date_epoch * 1000
			).toLocaleDateString("en-US", { month: "short", day: "numeric" });
		}
		if (forecastDay2) {
			date_day2 = new Date(
				forecastDay2.date_epoch * 1000
			).toLocaleDateString("en-US", { month: "short", day: "numeric" });
		}
		if (forecastDay3) {
			date_day3 = new Date(
				forecastDay3.date_epoch * 1000
			).toLocaleDateString("en-US", { month: "short", day: "numeric" });
		}
		...
		let daily_will_it_rain = forecastDay1?.day.daily_will_it_rain || "";

		let chance_rain_day1 =
			forecastDay1?.day.daily_chance_of_rain.toString() || "";
		let chance_rain_day2 =
			forecastDay2?.day.daily_chance_of_rain.toString() || "";
		let chance_rain_day3 =
			forecastDay3?.day.daily_chance_of_rain.toString() || "";
		...
		// Create the umbrella icon element
		...
		// Create the umbrella icon element
		const umbrellaIcon = createEl("div");
		umbrellaIcon.className = "weather-icon weather-umbrella-icon";
		setIcon(umbrellaIcon, "umbrella"); // Assuming you have an appropriate icon name for an umbrella
		umbrellaIcon.style.display =
			daily_will_it_rain === 1 ? "block" : "none"; // Display the umbrella icon if daily_will_it_rain is 1, otherwise hide it

		// Append the umbrella icon to the icon container
		iconContainer.appendChild(umbrellaIcon);

		// Create the temperature icon element
		// const temperatureIcon = createEl('div');
		// temperatureIcon.className = 'weather-icon weather-temperature-icon';
		// setIcon(temperatureIcon, isColdTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) ? 'thermometer-snowflake' : (isHotTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) ? 'thermometer-sun' : ''));
		// temperatureIcon.style.display = isColdTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) || isHotTemperature(Number(current.feelslike_c), Number(current.feelslike_f)) ? 'block' : 'none';

		const temperatureIcon = createEl("div");
		temperatureIcon.className = "weather-icon weather-temperature-icon";

		const sweatingIcon = document.createElement("img");

		if (
			isColdTemperature(
				Number(current.feelslike_c),
				Number(current.feelslike_f)
			)
		) {
			temperatureIcon.classList.add("cold-icon");
			setIcon(temperatureIcon, "thermometer-snowflake");
		} else if (
			isHotTemperature(
				Number(current.feelslike_c),
				Number(current.feelslike_f)
			) &&
			isHighHumidity(Number(current.humidity))
		) {
			temperatureIcon.classList.add("sweating-icon");
			// const sweatingIconDataUrl =
			// 	"data:image/svg+xml;base64,";
            // sweatingIcon.src = sweatingIconDataUrl;
            sweatingIcon.src = "https://github.com/ms3056/weather-icons/blob/dev/production/fill/svg/humidity.svg";
			sweatingIcon.alt = "Sweating Icon";
			sweatingIcon.style.width = "100px";
			sweatingIcon.style.height = "100px";
			temperatureIcon.appendChild(sweatingIcon);
		} else if (
			isHotTemperature(
				Number(current.feelslike_c),
				Number(current.feelslike_f)
			...
			temperatureIcon.classList.add("hot-icon");
			setIcon(temperatureIcon, "thermometer-sun");
		} else {
			temperatureIcon.style.display = "none";
		}

		// Create the humidity icon element
		// Create the humidity icon element
		const humidityIcon = createEl("div");
		humidityIcon.className = "weather-icon weather-humidity-icon";
		setIcon(
			humidityIcon,
			isHighHumidity(Number(current.humidity)) &&
				!isHotTemperature(
		...
		iconContainer.appendChild(temperatureIcon);
		iconContainer.appendChild(humidityIcon);

		// Append the icon container to the main container element
		containerEl.appendChild(iconContainer);
		...
		forecastDay1DateEl.textContent =
			date_day1 ===
		...
		lastUpdatedEl.classList.add("update-time");
		lastUpdatedEl.textContent = `Source Updated: ${current.last_updated}`;
		containerEl.appendChild(lastUpdatedEl);
...
class WeatherSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				const inputEl = text
					...
					.setValue(this.plugin.settings.location).inputEl;
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Set the refresh rate in minutes. Enter 999 to disable the refresh."
			)
			.addText((text) =>
				text
